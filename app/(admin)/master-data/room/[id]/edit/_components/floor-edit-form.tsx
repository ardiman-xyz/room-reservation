...
import {create, updateFloor} from "@/actions/floor"
...
export const FloorEditForm = ({buildings, defaultData}: IProps) => {
    ...
    const form = useForm<z.infer<typeof FloorSchema>>({
        ...
        defaultValues: {
            building: defaultData.buildingId,
    ...
    function onSubmit(values: z.infer<typeof FloorSchema>) {
        ...
        startTransition(() => {
            updateFloor(values, defaultData.id).then((data) => {
                setError(data?.error);
                if(data?.success) {
